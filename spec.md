...
- **不存在 `let`/`var` 或 `const` 关键字式声明**：`const`/`copyd` 是修饰符而非声明关键字，必须连同类型写在名字之前。`let x = 1;`、`var y = 2;`、`const N = 3;`（缺类型）一律非法；正典写法 `int x = 1;`、`const int N = 3;`。规范、示例、模板、补全与文档一律不得出现 `let`/`var` 形态。
...
- 声明**没有关键字式形态**：不存在 `let`/`var` 声明，也不存在省略类型的 `const N = 3;`（`const`/`copyd` 只是写在类型前的修饰符）。
