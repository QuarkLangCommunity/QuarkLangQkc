...
> **唯一正典形态：修饰 + 类型 + 名字（类型在前）。** 旧「名字在前」写法已删除，不保留兼容。
> **不存在 `let`/`var` 或 `const` 关键字式声明**：`const`/`copyd` 是**修饰符**，必须连同类型一起写在名字之前
...
> **K-note（声明形态红线）**：声明永远是「修饰 + 类型 + 名字」，**没有 `let`/`var` 关键字式声明**，
> 也不存在省略类型的 `const N = 3;`（`const` 只是修饰符）。规范、示例、模板、补全、文档一律不得出现
> `let`/`var` 声明形态；正典写法：
>
> ```qk
